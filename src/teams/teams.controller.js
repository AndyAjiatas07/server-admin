...
export const getTeamById = async (req, res) => {
  try {
    ...
    const team = await Team.findById(id);  // Eliminamos el .populate()
...
export const createTeam = async (req, res) => {
  try {
    ...
    // Verificamos si se sube una imagen de logo
    if (req.file) {
...
export const updateTeam = async (req, res) => {
  try {
    ...
    // Si hay una nueva imagen, actualizamos el logo
    if (req.file) {
      const currentTeam = await Team.findById(id);

      if (currentTeam?.logo) {
        const publicId = `kinal_sports/${currentTeam.logo.split('.')[0]}`;
...
export const changeTeamStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Equipo ${isActive ? 'activado' : 'desactivado'} exitosamente`,
      data: team,
    });
  } catch (error) {
    res.status(500).json({
